...
- **Ta source est un document reçu (pas une page web)** — réponse CADA par email, budget papier scanné… : deux circuits (politique [#14](https://github.com/MrPognon/MonPognon/blob/main/data-sources/documents/README.md)) : si la demande est passée par **madada.fr**, cite l'URL madada de la réponse ; sinon le document caviardé se versionne dans `data-sources/documents/` avec sa provenance, et `source.url` pointe ce fichier du dépôt. Dis-le à ton IA : elle préparera la provenance avec toi.
