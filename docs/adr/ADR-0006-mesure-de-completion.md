...
## Note d'application — 2026-07-21 : `plafond-legal.json` livré, et il ne ferme rien

Le §5 prévoyait ce fichier « avec fondement juridique cité à l'article ». Il est écrit. Cinq fondements instruits (droit d'accès, défense, vie privée, secret des affaires et fiscal, secret statistique), chacun soumis à double réfutation (sur-fermeture / écart droit-pratique), chaque article ouvert au texte sur Légifrance.

**Résultat, et c'est le résultat lui-même qui compte : dix-neuf lignes de fermeture proposées, dix-neuf abattues. Aucune `ferme-droit` n'a survécu.** Le fichier livré ne contient que deux entrées, toutes deux `communicable-sur-demande` (marché public, petite collectivité hors open-data).

Le motif de rejet est presque toujours le même, et il est instructif : on prend une protection qui **restreint le cercle des destinataires** (CRPA L311-6 : « ne sont communicables qu'à l'intéressé ») ou qui **porte sur une mention occultable** (CRPA L311-7 : communication après occultation), et on la lit comme une interdiction générale de publier. C'est la même classe d'erreur que les six pièges de périmètre du dépôt : deux notions voisines dont le nom se ressemble — ici « non communicable à un tiers » et « fermé par le droit ».

Conséquences durables, désormais mécaniques :

- `plafond-legal.json` est chargé **après** `indice_cp()` et n'entre dans aucun quotient — vérifié par construction : C et P sont identiques avec et sans le fichier.
- `valider_plafond()` refuse une entrée sans citation d'article, une URL non-Légifrance, un article `abroge: true`, un `effet_bouton` incohérent avec le statut, une portée sur un bloc inconnu. **Déclarer une fermeture sans texte est impossible.**
- Le bouton « réclamer » est **fail-open** : l'absence d'entrée le laisse allumé. Le principe est la communication (L311-1) ; une extinction est un acte positif, écrit, cité, daté. À ce jour, zéro extinction.
