...
- `data/**/*.json` — source de vérité. Un arbre par fichier, nœuds conformes à `schema/noeud.schema.json`. Chaque racine de fichier déclare `bloc_univers`, `volet` et une table `niveaux` (ADR-0006). ⚠️ **Les `schema/*.json` sont documentaires : aucun script ne les charge.** `build.py` valide à la main (`validate`, `valider_flux`, `valider_denominateur`, `valider_racine_cp`) — toucher au validateur sans toucher au schéma les fait diverger en silence.
...
- `docs/adr/` — 6 ADR acceptés. **Les lire avant tout travail structurant.**
