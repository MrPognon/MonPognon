...
- `data/etat/subventions/<prog>.json` — **107 fragments**, un par programme, portant les **112 722 bénéficiaires** de subventions aux associations avec SIREN et `rattachement_id`. Routés comme les fiches (ADR-0004) : publiés en fragments, **jamais dans `data.js`** (ils y ajouteraient ~31 Mo), mais présents dans `arbres` — `indice_cp()` lit les arbres en mémoire, pas le fichier publié. Racine à `bloc_univers: null` : vue transverse, elle ne compte pas dans C et ne sert qu'au reclassement P5.
...
- `data/plafond-legal.json` — **registre du contexte juridique** (ADR-0006 §5), schéma `schema/plafond-legal.schema.json`. Chaque entrée est citée à l'article, mot pour mot, avec l'URL Légifrance et la date de version en vigueur. **N'entre dans aucun quotient** (chargé après `indice_cp()`) et gate le bouton « réclamer ». `valider_plafond()` interdit une fermeture sans texte. État au 07/2026 : revue de 5 fondements, **19 fermetures proposées, 19 abattues, zéro `ferme-droit`** — le droit ne ferme absolument aucune donnée de l'arbre ; les « secrets » invoqués portent sur des mentions occultables (CRPA L311-7) ou restreignent les destinataires (L311-6), pas sur les montants.
...
- **P est achetable si on n'y prend pas garde.** Poser `niveaux[6] = "P4"` sur `data/etat/depenses.json` ferait gagner **+0,201 de P pour un caractère** — et ce serait faux : la profondeur 6 porte les *titres LOLF*, une nature de dépense, pas un organisme destinataire. Le `null` actuel est correct (ADR-0006, note 4). Depuis le 21/07/2026, `valider_sur_rattachement()` refuse par ailleurs qu'une ligne payeuse reçoive plus de 3× ce qu'elle porte — sans ce contrôle, 100 Md€ pointés sur un programme de 2,9 Md€ passaient la CI et rapportaient +0,16 de P.
- **Le taux de change de P : 1 Md€ reclassé de P3 à P5 vaut +0,0016 point.** Un dixième de point exige ~42 Md€. Le gain total encore disponible sur l'État est de **+0,023** — P est proche de son plafond, comme C.
...
`build.py --check` sort en 0 mais émet **une dizaine d'AVERTISSEMENT non bloquants** : ce n'est pas une régression de ta PR. Ils sont réels — des montants sans `source.maj` sur `coll.depenses` / `coll.recettes` et leurs sous-postes. Les anciens faux positifs sur montants négatifs (« somme des enfants (-4,295) > parent (-4,295) ») ont été supprimés : le contrôle « somme des enfants > parent » ne s'applique plus qu'aux parents positifs (`build.py`, `validate()`).
...
- **Collectivités** — **56 491 fiches** : 34 877 communes, 1 266 intercommunalités, 97 départements, 17 régions, 8 743 syndicats, 11 394 CCAS/CIAS, 97 SDIS. Comptes 2024 — sauf départements et régions, passés à 2025 —, réconciliés au centime par le pipeline (`centime()`, tolérance 0,01 €) ; à défaut de réconciliation, total seul sans ventilation.
...
- **L'OFGL publie les échelons à des rythmes différents.** Au 21/07/2026 : l'exercice 2025 est publié pour les départements et les régions — **ingérés à 2025** (`ecart_millesime: false`) — ainsi que pour les CCAS et SDIS, laissés **ingérés à 2024** car leur bloc `APUL.odal` n'est pas compté dans C (les rafraîchir ne bouge ni C ni P — cf. corollaire ci-dessus). Il **n'existe pas encore** pour les communes, les intercommunalités ni les syndicats (jeu communes mis à jour le 15/07/2026, il s'arrête à 2024). `fiches_echelons_ofgl.py` prend donc `--exercice` ; le défaut reste 2024. `ecart_millesime: true` subsiste sur `APUL.communes` (168,0 Md€) et `APUL.syndicats` (15,7 Md€) — dette déclarée, refermable dès publication.
...
Également ouvert : le passage à 2025 des communes, intercommunalités et syndicats, en attente de publication OFGL. Les pages `/perimetre` · `/plafond` · `/methode` et `data/plafond-legal.json` sont désormais livrées et relues juridiquement (ADR-0006 §5, revue au 07/2026 : 5 fondements, 19 fermetures proposées, 19 abattues, zéro `ferme-droit`). Le registre ne porte d'ailleurs aucun montant — sa livraison ne dépend d'aucune vérification chiffrée.
